...
(function ($) {
    ...
    w2grid.prototype = {
        msgDelete       : 'Estas seguro que quieres eliminar el Registro?',
        ...
        buttons: {
            'reload'   : { type: 'button', id: 'w2ui-reload', icon: 'w2ui-icon-reload', tooltip: 'Recargar datos' },
            'columns'  : { type: 'drop', id: 'w2ui-column-on-off', icon: 'w2ui-icon-columns', tooltip: 'Mostrar/Ocultar Columnas', arrow: false, html: '' },
            ...
            'add'      : { type: 'button', id: 'w2ui-add', text: 'Agregar', tooltip: 'Agregar', icon: 'w2ui-icon-plus' },
            'edit'     : { type: 'button', id: 'w2ui-edit', text: 'Editar', tooltip: 'Editar', icon: 'w2ui-icon-pencil', disabled: true },
            'delete'   : { type: 'button', id: 'w2ui-delete', text: 'Rechazar', tooltip: 'Rechazar', icon: 'w2ui-icon-cross', disabled: true },
        ...
        searchReset: function (noRefresh) {
            ...
            // --- do not reset to Todas las columnas (I think)
            if (this.searches.length > 0) {
                if (!this.multiSearch || !this.show.searchAll) {
                    ...
                    this.last.field   = 'all';
                    this.last.caption = w2utils.lang('Todas las columnas');
                }
            }
        ...
        searchShowFields: function () {
            ...
            for (var s = -1; s < this.searches.length; s++) {
                ...
                if (s == -1) {
                    ...
                    search = { field: 'all', caption: w2utils.lang('Todas las columnas') };
                } else {
        ...
        initAllField: function (field, value) {
            var el = $('#grid_'+ this.name +'_search_all');
            if (field == 'all') {
                var search = { field: 'all', caption: w2utils.lang('Todas las columnas') };
                el.w2field('clear');
                el.change();
            } else {
                var search = this.getSearch(field);
        ...
        render: function (box) {
            ...
            if (!this.last.field) {
                if (!this.multiSearch || !this.show.searchAll) {
                    ...
                    this.last.field   = 'all';
                    this.last.caption = w2utils.lang('Buscar');
                }
            }
            // insert elements
            $(this.box)
        ...
        initColumnOnOff: function () {
            ...
            var col_html = '<div class="w2ui-col-on-off">'+
                ...
                '      <label>'+ w2utils.lang('Lineas #') +'</label>'+
                '   </td>'+
                '</tr>';
            // columns
            for (var c = 0; c < this.columns.length; c++) {
            ...
            if (obj.show.saveRestoreState) {
                col_html += '<tr><td colspan="2" onclick="var obj = w2ui[\''+ obj.name +'\']; obj.toolbar.uncheck(\'w2ui-column-on-off\'); obj.stateSave();">'+
                            '    <div style="cursor: pointer; padding: 4px 8px; cursor: default">'+ w2utils.lang('') + '</div>'+
                            '</td></tr>'+
        ...
        scroll: function (event) {
            ...
            $('#grid_'+ this.name + '_footer .w2ui-footer-right').html(
                (obj.show.statusRange ? w2utils.formatNumber(this.offset + t1) + '-' + w2utils.formatNumber(this.offset + t2) +
                        (this.total != -1 ? ' ' + w2utils.lang('de') + ' ' +    w2utils.formatNumber(this.total) : '') : '') +
